class Circle {   // Moved Circle class to new tab.
  ...
  Circle(PVector _loc, float _radius){
    loc = _loc;
    radius = _radius;
    c = lerpColor(c1, c2, sq(loc.mag() / 400));   // Changed the color changed within the circles.
  ...
  float easeOutElastic(float t, float b, float c, float d){
    ...
    return a * pow(2, -11 * t) * sin((t * d - s) * (2 * PI) / p) + c + b;  // Changed the speed that it eases back in and out.
  }
}